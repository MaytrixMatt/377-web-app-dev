// This isnt cheating - this is just electing not to use python
var rucksacks = [
...
// Start part one
var compartmentSimilarities = [];
function partUno(){
    for(var i = 0; i < rucksacks.length; i++){

        var rucksackLength = rucksacks[i].length;

        //var sub1 = rucksacks[i].substr(0, length/2);
        //var sub2 = rucksacks[i].substr(length/2, length);

        while(true){

            for(var j = (rucksackLength/2); j < rucksackLength ; j++){
                
                for(var k = 0; k < (rucksackLength/2); k++ ){
    
                    if(rucksacks[i].substr(j) == rucksacks[i].substr(k)){
                        
                        compartmentSimilarities.push(rucksacks[i].substr(j));
                        break;
    ...
    console.log(compartmentSimilarities);
}
partUno();